...
@objcMembers open class ChatMessageCell: UITableViewCell {
    ...
    @objc open func createContainer() -> UIView {
        UIView(frame: CGRect(x: Appearance.chatAreaSidePadding, y: 6, width: self.contentView.frame.width - Appearance.chatAreaSidePadding*2, height: self.frame.height - 6)).backgroundColor( UIColor.theme.barrageLightColor2).cornerRadius(.small)
    }
